...
oo::class create PDA {
    ...
    constructor args {
        ...
        set g [::struct::graph]
        foreach {from edge to} $transitions {
            ::log::log d [list $from $edge $to]
            foreach state [list $from $to] {
                if {![$g node exists $state]} {
                    $g node insert $state
                }
            }
            lassign [split $edge /] input output
    ...
    method MooreEnter {states symbols stack} {
        # TODO Moore hook / entry
        if {[llength $symbols] <= 0} {
            return [expr {intersectp($states, $accepting)}]
        } else {
            return [my Mealy $states $symbols $stack]
        }
    }

    method Mealy {states symbols stack} {
        ...
        my MooreExit $arcs $symbols $stack
    }

    method MooreExit {arcs symbols stack} {
        set result [lmap arc $arcs {
            if no {
                # TODO Moore exit
                set state [$g arc source $arc]
            }
    ...
    method accept {start symbols stack} {
        log::log d [info level 0] 
        my MooreEnter [my ε-moves $start $stack] $symbols $stack
    }

...
package require Tcl 8.6		;# -verbose line uses [info frame]
namespace eval tcltest {

    # When the version number changes, be sure to update the pkgIndex.tcl file,
    # and the install directory in the Makefiles.  When the minor version
    # changes (new feature) be sure to update the man page as well.
    variable Version 2.4.666
...
##### Set up the configurable options
    ...
    Option -encoding [encoding system] {
        Set encoding for sourced files.
    } AcceptList srcenc 
...
proc tcltest::runAllTests { {shell ""} } {
	...
	if {[singleProcess]} {
	    incr numTestFiles
	    uplevel 1 [list ::source -encoding [srcenc] $file]
	} else {
	    ...
	    set cmd [linsert $childargv 0 | $shell -encoding [srcenc] $file]
	    if {[catch {
		incr numTestFiles
	...
	uplevel 1 [list ::source -encoding [srcenc] [file join $directory all.tcl]]
